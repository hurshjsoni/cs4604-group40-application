...
export const getAnalytics = query({
  ...
  handler: async (ctx) => {
    ...
    // Listing rent distribution (active listings only)
    const rentRanges = [
    ...
    const activeListingsList = listings.filter((l) => l.isActive);
    const listingsByRentRange = rentRanges.map((range) => ({
      range: range.label,
      count: activeListingsList.filter(
        (l) => l.rent >= range.min && l.rent < range.max,
      ).length,
    }));

    // Listing bedroom distribution (active listings only)
    const bedroomCounts: Record<string, number> = {};
    for (const listing of activeListingsList) {
      const label = listing.bedrooms === 0 ? "Studio" : `${listing.bedrooms} BR`;
      bedroomCounts[label] = (bedroomCounts[label] ?? 0) + 1;
    }
    const listingsByBedrooms = Object.entries(bedroomCounts)
