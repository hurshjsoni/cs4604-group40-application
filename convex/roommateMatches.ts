...
export const getForUser = query({
  ...
  handler: async (ctx) => {
    ...
    // don't show the same person twice if the reverse record also exists.
    const coveredUserIds = new Set(outgoing.map((m) => m.matchedUserId));
    ...
    const incomingAsSender = incomingAccepted
    ...
    const allMatches = [...outgoing, ...incomingAsSender];
